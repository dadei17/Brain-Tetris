...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		for(int i=0; i<points.length; i++) body[i] = points[i];
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for(int i=0; i<body.length; i++) {
			TPoint p = new TPoint(body[i].y,body[i].x);
			newPoints[i] = p;
			p.x = height - newPoints[i].x - 1;
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		TPoint[] points = other.getBody();
		
		int fullmatched = body.length;
		for(int i =0; i<points.length; i++) {
			TPoint cur = points[i];
			for(int j =0; j<body.length; j++) {
				if(cur.x == body[j].x && cur.y == body[j].y) {
					fullmatched--;
				}
			}
		}
		if(fullmatched != 0) return false;
		return true;
	}
